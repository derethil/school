...
void simpleExample()
{
    ...
    auto f = std::find_if(pq.begin(), pq.end(), [](auto value)
...
void simulation()
{
    ...
    while (!pq.empty())
    {
        ...
        auto boostMe = std::find_if(pq.begin(), pq.end(), [=](auto item)
                                    { return item.priority >= priorityMin && item.priority <= priorityMax; });
        if (boostMe != pq.end())
        {
